...
subroutine AppendCropFilePerennials(totalname, GenrateTheOnset, GenerateTheEnd, &
    ...
    integer :: criterionNronset_local
    integer :: criterionNrend_local
    integer :: occurrenceonset_local
    ...
    genrateend_local = GenerateTheEnd
    ...
    ! 2. ONSET
    ! criterion number
    if (genrateonset_local .eqv. .false.) then
        criterionNronset_local = 0
        TempString = '         : The Restart of growth is fixed on a specific date'
    else
        select case (criterionNronset_local)
            case(12)
                TempString = &
            '         : The Restart of growth is generated by average air temperature'
            case(13)
                TempString = &
            '         : The Restart of growth is generated by Growing-degree days'
            case default
                ! no valid criterion number
                genrateonset_local = .false.
                criterionNronset_local = 0
                TempString = '         : The Restart of growth is fixed on a specific date'
        end select
    end if
    write(fhandle, '(i6, a)') criterionNronset_local, TempString

    ! parameters ONSET
    if (genrateonset_local .eqv..false.) then
        write(fhandle, '(i6, a)') Day1Onset, '         : First Day of growth'
        write(fhandle, '(i6, a)') Month1Onset, '         : First Month of growth'
        write(fhandle, '(i6, a)') LengthOnset, &
        '         : Length (days) for the time window (Restart of growth): Not Applicable'
        write(fhandle, '(f8.1, a)') ThresholdOnset, &
        '       : Threshold for the Restart criterion: Not Applicable'
        write(fhandle, '(i6, a)') SuccessiveDaysOnset, &
        '         : Number of successive days for the Restart criterion: Not Applicable'
        write(fhandle, '(i6, a)') occurrenceonset_local, &
        '         : Number of occurrences before the Restart criterion applies: Not Applicable'
    else
        write(fhandle, '(i6, a)') Day1Onset, &
                '         : First Day for the time window (Restart of growth)'
        write(fhandle, '(i6, a)') Month1Onset, &
                '         : First Month for the time window (Restart of growth)'
        write(fhandle, '(i6, a)') LengthOnset, &
                '         : Length (days) of the time window (Restart of growth)'
        select case (criterionNronset_local)
            case(12)
                TempString = &
        '       : Threshold for the Restart criterion: Average air temperature (degC)'
            case(13)
                TempString = &
                '       : Threshold for the Restart criterion: Growing-degree days'
        end select
    end if
    write(fhandle,  '(f8.1, a)') ThresholdOnset, TempString
    write(fhandle, '(i6, a)') SuccessiveDaysOnset, &
                '         : Number of successive days for the Restart criterion'
    if (occurrenceonset_local > 3) then
        occurrenceonset_local = 3
    end if
    write(fhandle, '(i6, a)') occurrenceonset_local, & 
        '         : Number of occurrences before the Restart criterion applies'

    ! 3. END of growing period
    ! criterion number
    if (genrateend_local .eqv. .false.) then
        criterionNrend_local = 0
        TempString = '         : The End of growth is fixed on a specific date'
    else
        select case (criterionNrend_local)
            case(62)
                TempString = &
            '         : The End of growth is generated by average air temperature'
            case(63)
                TempString = &
            '         : The End of growth is generated by Growing-degree days'
            case default
                ! no valid criterion number
                genrateend_local = .false.
                criterionNrend_local = 0
                TempString = '         : The End of growth is fixed on a specific date'
        end select
    end if
    write(fhandle, '(i6, a)') criterionNrend_local, TempString

    ! parameters END of growing period
    if (genrateend_local .eqv. .false.) then
        write(fhandle, '(i6, a)') DayNEnd, '         : Last Day of growth'
        write(fhandle, '(i6, a)') MonthNEnd, '         : Last Month of growth'
        write(fhandle, '(i6, a)') ExtraYearEnd, &
        '         : Number of years to add to the Restart year'
        write(fhandle, '(i6, a)') LengthEnd, &
        '         : Length (days) for the time window (End of growth): Not Applicable'
        write(fhandle, '(f8.1, a)') ThresholdEnd, &
        '       : Threshold for the End criterion: Not Applicable'
        write(fhandle, '(i6, a)') SuccessiveDaysEnd, &
        '         : Number of successive days for the End criterion: Not Applicable'
        write(fhandle, '(i6, a)') occurrenceend_local, &
        '         : Number of occurrences before the End criterion applies: Not Applicable'
    else
        write(fhandle, '(i6, a)') DayNEnd, &
        '         : Last Day for the time window (End of growth)'
        write(fhandle, '(i6, a)') MonthNEnd, &
        '         : Last Month for the time window (End of growth)'
        write(fhandle, '(i6, a)') ExtraYearEnd, &
        '         : Number of years to add to the Onset year'
        write(fhandle, '(i6, a)') LengthEnd, &
        '         : Length (days) of the time window (End of growth)'
        select case (criterionNrend_local)
            case(62)
                TempString = &
        '       : Threshold for the End criterion: average air temperature (degC)'
            case(63)
                TempString = &
        '       : Threshold for the End criterion: Growing-degree days'
        end select
    end if
    write(fhandle, '(f8.1, a)')  ThresholdEnd, TempString
    write(fhandle, '(i6, a)')  SuccessiveDaysEnd, &
    '         : Number of successive days for the End criterion'
    if (occurrenceend_local > 3) then
        occurrenceend_local = 3
    end if
    write(fhandle, '(i6, a)') occurrenceend_local, &
    '         : Number of occurrences before the End criterion applies'
    Close(fhandle)
