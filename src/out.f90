...
ElapsedDays : ARRAY[1..12] OF real(dp) = (0,31,59.25,90.25,120.25,151.25,181.25,212.25,243.25,273.25,304.25,334.25)
...
type rep_Content 
    !!  total water (mm) or salt (Mg/ha) content
    real(dp) :: BeginDay
...
rep_int_array = ARRAY[1..4] OF integer(int32)
...
type rep_OUTindividual 
    ...
    logical :: Requested
        !! Undocumented
end type rep_OUTindividual 


type rep_OUTevaluation 
    logical :: EvalDataOUT
        !! Undocumented
    logical :: EvalStatOUT
        !! Undocumented
    logical :: Requested
        !! Undocumented
end type rep_OUTevaluation 


type rep_OUTfiles 
    logical :: FileOUTseasonal
        !! Undocumented
    type(rep_OUTindividual) :: FileOUTindividual
        !! Undocumented
    type(rep_OUTevaluation) :: FileOUTevaluation
        !! Undocumented
end type rep_OUTfiles 



character(len=STRING_LENGTH) :: PathNameProg, PathNameData, PathNameOutp, PathNameSimul, PathNameObs, PathNameImport
logical :: DataPath, ObsPath
character(len=STRING_LENGTH) :: ProfFile, CalendarFile, CropFile, ClimateFile, ClimFile, EToFile, RainFile, TemperatureFile, CO2File, IrriFile, ManFile, SWCiniFile, ProjectFile, MultipleProjectFile, OffSeasonFile, GroundWaterFile, ObservationsFile
character(len=STRING_LENGTH) :: ProfFilefull, CalendarFileFull, CropFilefull, ClimateFileFull, EToFilefull, RainFileFull, TemperatureFileFull, CO2FileFull, IrriFileFull, ManFileFull, SWCiniFileFull, ProjectFileFull, MultipleProjectFileFull, OffSeasonFileFull, GroundWaterFileFull, ObservationsFileFull, FullFileNameProgramParameters
character(len=STRING_LENGTH) :: ProfDescription, ClimateDescription, CalendarDescription, CropDescription, ClimDescription, EToDescription, RainDescription, TemperatureDescription, CO2Description, IrriDescription, ManDescription, SWCiniDescription, ProjectDescription, MultipleProjectDescription, OffSeasonDescription, GroundWaterDescription, ObservationsDescription
type(rep_clim) :: ClimRecord, EToRecord, RainRecord, TemperatureRecord
type(rep_sim) :: Simulation
type(rep_IrriMode) :: IrriMode
type(rep_IrriMethod) :: IrriMethod
type(rep_IrriECw) :: IrriECw
type(rep_GenerateTimeMode) :: GenerateTimeMode
type(rep_GenerateDepthMode) :: GenerateDepthMode
integer(int32) :: IrriFirstDayNr
type(rep_SoilLayer) :: SoilLayer
type(rep_Comp) :: Compartment
type(rep_soil) :: Soil
integer(int32) :: NrCompartments
type(rep_Content) :: TotalWaterContent, TotalSaltContent
    !! Water Content (mm) and Salt Content (Mg/ha)
type(rep_Crop) :: Crop
...
type(repTypeClimData) :: TypeClimData
integer(int32) :: MaxPlotNew
type(rep_TypePlot) :: TypePlotNew
integer(int8) :: MaxPlotTr
type(rep_OUTfiles) :: OUTPUTfiles
...
subroutine DetermineLengthGrowthStages(CCoVal, CCxVal, CDCVal, L0, TotalLength, CGCgiven, TheDaysToCCini, ThePlanting, Length123, StLength, Length12, CGCVal)
    ...
    type(rep_planting), intent(in) :: ThePlanting
    integer(int32), intent(inout) :: Length123
    type(rep_int_array), intent(inout) :: StLength
...
subroutine GetCO2Description(CO2FileFull, CO2Description)
    character(len=STRING_LENGTH), intent(in) :: CO2FileFull
    character(len=STRING_LENGTH), intent(inout) :: CO2Description
...
Simulation%SCor = 1
if (nint(Soil%RootMax*1000) < nint(Zmax*1000)) then
    ZrAdjustedToRestrictiveLayers(Zr, Soil%NrSoilLayers, SoilLayer, Zr)
end if
! assign
...
READLN(f0, i)
if (i = 1) then
    RunoffON = .false.   ! prevention of surface runoff
else
    RunoffON = .true.   ! surface runoff is not prevented
end if
if (nint(VersionNr*10) < 50) then ! UPDATE required for CN adjustment
    CNcorrection = 0
else
    READLN(f0, CNcorrection) ! project increase/decrease of CN
end if
! weed infestation
...
if (nint(VersionNr*10) < 70) then ! UPDATE required for versions below 7
    WeedAdj = 100 ! replacement (%) by weeds of the self-thinned part of the Canopy Cover - only for perennials
else
    READLN(f0, WeedAdj)
end if
! multiple cuttings
...
do layeri = (Soil%NrSoilLayers+1), max_No_compartments 
...
integer(int32) :: i
do i= (Soil%NrSoilLayers+1), max_SoilLayers 
...
if (StLength(1) > TotalLength) then
    ...
    if ((StLength(1)+StLength(2)) > TotalLength) then
        ...
        if ((StLength(1)+StLength(2)+StLength(3)) > TotalLength) then
            ...
            StLength(4) = 0
        elseif ((StLength(1)+StLength(2)+StLength(3)+StLength(4)) > TotalLength) then
...
if (Crop%ModeCycle = CalendarDays) then
    ...
    if (Management%FertilityStress /= 0) then
...
Dayi = nint(SumDayMonth - ElapsedDays(Monthi) + 0.25 + 0.06)
! DetermineDate 
...
if ((NOT SimulParam%ConstGwt) .and. (IniSimFromDayNr /= Simulation%FromDayNr)) then
    if (GroundWaterFile = '(None)') then
        FullFileName = CONCAT(PathNameProg, 'GroundWater%AqC')
    else
        FullFileName = GroundWaterFileFull
    end if
...
if ((RootingDepth*100) <= SimulParam%ThicknessTopSWC) then
    RootZoneWC%ZtopAct = RootZoneWC%Actual
    RootZoneWC%ZtopFC = RootZoneWC%FC 
    RootZoneWC%ZtopWP = RootZoneWC%WP
    RootZoneWC%ZtopThresh = RootZoneWC%Thresh
else
    CumDepth = 0
    compi = 0
    RootZoneWC%ZtopAct = 0
    RootZoneWC%ZtopFC = 0
    RootZoneWC%ZtopWP = 0
    RootZoneWC%ZtopThresh = 0
    ...
    RootZoneWC%ZtopAct = RootZoneWC%ZtopAct
    + Factor * 1000 * Compartment(compi)%Theta * Compartment(compi)%Thickness
    * (1 - SoilLayer(Compartment(compi)%Layer)%GravelVol/100)
    RootZoneWC%ZtopFC = RootZoneWC%ZtopFC
    + Factor * 10 * SoilLayer(Compartment(compi)%Layer)%FC * Compartment(compi)%Thickness
    * (1 - SoilLayer(Compartment(compi)%Layer)%GravelVol/100)
    RootZoneWC%ZtopWP = RootZoneWC%ZtopWP
    + Factor * 10 * SoilLayer(Compartment(compi)%Layer)%WP * Compartment(compi)%Thickness
    * (1 - SoilLayer(Compartment(compi)%Layer)%GravelVol/100)
    RootZoneWC%ZtopThresh = RootZoneWC%ZtopThresh
    + Factor * 10 * Compartment(compi)%Thickness * (SoilLayer(Compartment(compi)%Layer)%FC
    - Crop%pActStom * (SoilLayer(Compartment(compi)%Layer)%FC-SoilLayer(Compartment(compi)%Layer)%WP))
    * (1 - SoilLayer(Compartment(compi)%Layer)%GravelVol/100)
...
if (nint(1000*(RootZoneWc%FC - RootZoneWc%WP)) > 0 then
    DrRel = (RootZoneWc%FC - RootZoneWC%Actual)/(RootZoneWc%FC - RootZoneWc%WP)
else
    DrRel = 0
end if
if (nint(1000*(RootZoneWC%ZtopFC - RootZoneWc%ZtopWP)) > 0 then
    DZtopRel = (RootZoneWC%ZtopFC - RootZoneWc%ZtopAct)/(RootZoneWC%ZtopFC - RootZoneWc%ZtopWP)
else
    DZtopRel = 0
end if
...
if (a = undef_int) then
    if (nint(b) = undef_int) then
        HIadjWStressAtFlowering = 1
    elseif (KsSto > 0.001) then
        HIadjWStressAtFlowering = (Exp(0.10*Ln(KsSto))) * (1-(1-KsSto)/b)
    else
        HIadjWStressAtFlowering = 0
    end if
else
    if (nint(b) = undef_int) then
        HIadjWStressAtFlowering = (1 + (1-KsVeg)/a)
    elseif (KsSto > 0.001) then
        HIadjWStressAtFlowering = (1 + (1-KsVeg)/a) * (Exp(0.10*Ln(KsSto))) * (1-(1-KsSto)/b)
    else
        HIadjWStressAtFlowering = 0
    end if
end if
! HIadjWStressAtFlowering 
...
EToStandard = 5
real(dp) :: SumGDD, GDDi, SumKcPot, KsB, SumGDDforPlot, SumGDDfromDay1
...
TotDepthL = 0
do layeri = 1 to Soil%NrSoilLayers 
    TotDepthL = TotDepthL + SoilLayer(layeri)%Thickness
end do
if (TotDepthC > TotDepthL) then
    SoilLayer(Soil%NrSoilLayers)%Thickness = SoilLayer(Soil%NrSoilLayers)%Thickness + (TotDepthC - TotDepthL)
end if
