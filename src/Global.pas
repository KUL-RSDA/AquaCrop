...
PROCEDURE TranslateIniPointsToSWProfile(NrLoc : ShortInt;
                                        ...
                                        NrComp : INTEGER;
                                        VAR Comp : rep_Comp);
PROCEDURE SplitStringInThreeParams(StringIN : string;
...
specify_soil_layer(NrCompartments,GetSoil().NrSoilLayers,SoilLayer,Compartment,TotalWaterContent);
...
IF ROUND(1000*(RootZoneWc.FC - RootZoneWc.WP)) > 0
   THEN DrRel := (RootZoneWc.FC - RootZoneWC.Actual)/(RootZoneWc.FC - RootZoneWc.WP)
   ELSE DrRel := 0;
IF ROUND(1000*(RootZoneWC.ZtopFC - RootZoneWc.ZtopWP)) > 0
   THEN DZtopRel := (RootZoneWC.ZtopFC - RootZoneWc.ZtopAct)/(RootZoneWC.ZtopFC - RootZoneWc.ZtopWP)
   ELSE DZtopRel := 0;
...
VAR f : textfile;
    ...
    i : ShortInt;
BEGIN
FullName := CONCAT(PathNameSimul,'GetSoil().PAR');
Assign(f,FullName);
Reset(f);
...
Parami := 0;
// divide the line in parameters
WHILE ((i < LengthS) AND (Parami < 3)) DO
   BEGIN
   i := i + 1;
   CharA := StringIN[i];
   IF (CharA > #32) THEN
      ...
      IF (Parami = 1) THEN Par1 := StrToFloat(StringNumber);
      IF (Parami = 2) THEN Par2 := StrToFloat(StringNumber);
      IF (Parami = 3) THEN Par3 := StrToFloat(StringNumber);
      END; // next Parameter
   END; // end of line
END; (* SplitStringInThreeParams *)
