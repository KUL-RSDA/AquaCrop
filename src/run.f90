...
subroutine AdvanceOneTimeStep()
    ...
    if (((GetCrop_ModeCycle() == modeCycle_CalendarDays) .and. &
        ...
        if (((GetDayNri()-GetSimulation_DelayedDays()) >= GetCrop_Day1()) .and. &
            ...
            if ((GetZiAqua() >= 0) .and. (GetRootingDepth() > &
    ...
    if ((GetRootingDepth() > 0) .and. (GetDayNri() == GetCrop_Day1())) then
    ...
    if ((GetRootingDepth() > 0) .and. (GetDayNri() == GetCrop_Day1()) &
    ...
    if ((GetRootingDepth() > 0) .and. (GetNoMoreCrop() .eqv. .false.)) then
         SWCtopSoilConsidered_temp = GetSimulation_SWCtopSoilConsidered()
        call DetermineRootZoneWC(GetRootingDepth(), SWCtopSoilConsidered_temp)
        call SetSimulation_SWCtopSoilConsidered(SWCtopSoilConsidered_temp)
        ! temperature stress affecting crop transpiration
        if (GetCCiActual() <= 0.0000001) then
             KsTr = 1._dp
        else
             KsTr = KsTemperature(0._dp, GetCrop_GDtranspLow(), GetGDDayi())
        end if
        call SetStressTot_Temp(((GetStressTot_NrD() - 1)*GetStressTot_Temp() + &
                     100._dp*(1-KsTr))/real(GetStressTot_NrD(), kind=dp))
        ! soil salinity stress
         ...
         call SetStressTot_Salt(((GetStressTot_NrD() - 1)*GetStressTot_Salt()&
                + 100._dp*(1-GetRootZoneSalt_KsSalt()))/&
    ...
    if (GetCCiActual() > 0._dp) then
        ...
        if (GetStressLeaf() > - 0.000001_dp) then
            call SetStressTot_Exp(((GetStressTot_NrD() - 1)*GetStressTot_Exp() &
        ...
        if (GetTpot() > 0._dp) then
            ...
            if (StressStomata > - 0.000001_dp) then
                call SetStressTot_Sto(((GetStressTot_NrD() - 1) &
    ...
    if (GetWeedRCi() > - 0.000001_dp) then
        call SetStressTot_Weed(((GetStressTot_NrD() - 1)*GetStressTot_Weed() &
    ...
    if (GetOutDaily()) then
        !GDL20220423! call WriteDailyResults((GetDayNri() - GetSimulation_DelayedDays() &
        !!      - GetCrop_Day1()+1), WPi)
    end if
    if (GetPart2Eval() .and. (GetObservationsFile() /= '(None)')) then
    ...
    if (GetSimulation_SumEToStress() >= 0.1) then
